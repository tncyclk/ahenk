        ...
        try:
            if lockRootUser:
                ...
                if str(password).strip() != '':
                    ...
                    shadow_password = crypt.crypt(password)
                    # shadow_password = crypt.crypt(password)
                    result_code, p_out, p_err = self.execute_command(self.change_password.format('\'{}\''.format(shadow_password), self.username))
